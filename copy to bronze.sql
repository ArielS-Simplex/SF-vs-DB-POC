-- Load 3 days of data for performance testing (September 4, 5, 6)
COPY INTO poc.public.ncp_bronze
FROM
  @NCP/bpa.STP_BusinessAnalyticsQuery/ PATTERN = 'file2-bpa.STP_BusinessAnalyticsQuery-2025-09-04.*' 
FILE_FORMAT = (FORMAT_NAME = 'txt_format') ON_ERROR = CONTINUE;

COPY INTO poc.public.ncp_bronze
FROM
  @NCP/bpa.STP_BusinessAnalyticsQuery/ PATTERN = 'file2-bpa.STP_BusinessAnalyticsQuery-2025-09-05.*' 
FILE_FORMAT = (FORMAT_NAME = 'txt_format') ON_ERROR = CONTINUE;

COPY INTO poc.public.ncp_bronze
FROM
  @NCP/bpa.STP_BusinessAnalyticsQuery/ PATTERN = 'file2-bpa.STP_BusinessAnalyticsQuery-2025-09-06.*' 
FILE_FORMAT = (FORMAT_NAME = 'txt_format') ON_ERROR = CONTINUE;

-- SHOW STAGES; -- Commented out for now

-- DATABRICKS-STYLE APPROACH: Load raw, then parse
-- This mimics exactly how Databricks handles messy data

-- STEP 1: Create staging table for raw data (first load as strings)
...
-- STEP 2: Create single-field file format (treat each line as one field)
...
-- STEP 3: Load raw data - JUST 1 HOUR FROM 1 DAY FOR TESTING
...
PATTERN = 'file2-bpa.STP_BusinessAnalyticsQuery-2025-09-06_00-00-00.txt'  -- Just one specific file
FILE_FORMAT = (FORMAT_NAME = 'txt_format_raw') 
ON_ERROR = CONTINUE;

-- DEBUG: Check what was loaded into staging
SELECT 
    'STAGING TABLE CHECK' AS debug_step,
    COUNT(*) AS total_rows,
    COUNT(CASE WHEN raw_line IS NOT NULL THEN 1 END) AS non_null_rows,
    MIN(LENGTH(raw_line)) AS min_line_length,
    MAX(LENGTH(raw_line)) AS max_line_length
FROM poc.public.ncp_bronze_staging;

-- Show sample data
SELECT filename, DW_CREATED_AT, LEFT(raw_line, 100) AS sample_line 
FROM poc.public.ncp_bronze_staging 
LIMIT 5;

-- STEP 4: Create final bronze table with COMPLETE 185-column structure
CREATE OR REPLACE TABLE poc.public.ncp_bronze AS
SELECT 
    filename,
    ...
    SPLIT_PART(raw_line, '\t', 105) AS stored_credentials_mode,
    SPLIT_PART(raw_line, '\t', 106) AS avs_code,
    SPLIT_PART(raw_line, '\t', 107) AS is_3d,
    SPLIT_PART(raw_line, '\t', 108) AS credit_type_id,
    SPLIT_PART(raw_line, '\t', 109) AS subscription_step,
    SPLIT_PART(raw_line, '\t', 110) AS scheme_token_fetching_result,
    SPLIT_PART(raw_line, '\t', 111) AS browser_screen_height,
    SPLIT_PART(raw_line, '\t', 112) AS browser_screen_width,
    SPLIT_PART(raw_line, '\t', 113) AS filter_reason_id,
    SPLIT_PART(raw_line, '\t', 114) AS reason_code,
    SPLIT_PART(raw_line, '\t', 115) AS reason,
    SPLIT_PART(raw_line, '\t', 116) AS request_timestamp_service,
    SPLIT_PART(raw_line, '\t', 117) AS token_unique_reference_service,
    SPLIT_PART(raw_line, '\t', 118) AS response_timestamp_service,
    SPLIT_PART(raw_line, '\t', 119) AS api_type_service,
    SPLIT_PART(raw_line, '\t', 120) AS request_timestamp_fetching,
    SPLIT_PART(raw_line, '\t', 121) AS token_unique_reference_fetching,
    SPLIT_PART(raw_line, '\t', 122) AS response_timestamp_fetching,
    SPLIT_PART(raw_line, '\t', 123) AS api_type_fetching,
    SPLIT_PART(raw_line, '\t', 124) AS is_cryptogram_fetching_skipped,
    SPLIT_PART(raw_line, '\t', 125) AS is_external_scheme_token,
    SPLIT_PART(raw_line, '\t', 126) AS three_ds_server_trans_id,
    SPLIT_PART(raw_line, '\t', 127) AS gateway_id,
    SPLIT_PART(raw_line, '\t', 128) AS cc_request_type_id,
    SPLIT_PART(raw_line, '\t', 129) AS upo_id,
    SPLIT_PART(raw_line, '\t', 130) AS iscardReplaced,
    SPLIT_PART(raw_line, '\t', 131) AS isvdcuFeeApplied,
    SPLIT_PART(raw_line, '\t', 132) AS aftType,
    SPLIT_PART(raw_line, '\t', 133) AS secondarycccid,
    SPLIT_PART(raw_line, '\t', 134) AS transaction_duration,
    SPLIT_PART(raw_line, '\t', 135) AS authorization_req_duration,
    SPLIT_PART(raw_line, '\t', 136) AS firstInstallment,
    SPLIT_PART(raw_line, '\t', 137) AS periodicalInstallment,
    SPLIT_PART(raw_line, '\t', 138) AS numberOfInstallments,
    SPLIT_PART(raw_line, '\t', 139) AS installmentProgram,
    SPLIT_PART(raw_line, '\t', 140) AS installmentFundingType,
    SPLIT_PART(raw_line, '\t', 141) AS first_installment_usd,
    SPLIT_PART(raw_line, '\t', 142) AS periodical_installment_usd,
    SPLIT_PART(raw_line, '\t', 143) AS applicableScenarios,
    SPLIT_PART(raw_line, '\t', 144) AS cascading_ab_test_experimant_name,
...
-- DEBUG: Check final bronze table
SELECT 
    'FINAL BRONZE TABLE CHECK' AS debug_step,
    COUNT(*) AS total_rows,
    COUNT(CASE WHEN TRANSACTION_MAIN_ID IS NOT NULL THEN 1 END) AS valid_transactions,
...
SELECT 
    ...
    TRANSACTION_MAIN_ID,
    THREED_FLOW_STATUS,
    LEFT(raw_line, 50) AS sample_raw
FROM poc.public.ncp_bronze 
LIMIT 5;

-- BRONZE TABLE IS NOW READY FOR ETL PROCESSING! 